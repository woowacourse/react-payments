...
5. '숫자만 입력 가능합니다' 에러 메시지 제거
    ...
    - 추가 이슈) 숫자 길이 맞게 입력한 뒤 숫자 외 입력 -> 에러 상태로 표시됨 -> but 실제로는 숫자 외 입력이 필터링됐기 때문에 valid라고 판단해야 함 -> 이런 케이스도 처리 필요 (해결)
    - => 숫자 외 입력에 대한 예외처리 불필요해짐 (sanitize) + 이미 max length인 경우에는 숫자 외 입력 에러 안 뜸
