...
function CardNameFormContainer({ link }: { link: To }) {
  const navigator = useNavigate();
  const { inputtedInfo, cardDispatch, inputtedInfoDispatch } = useContext(
    CardContext
  ) as CardContextValue;

  const { cardNumber, cardName } = inputtedInfo;

  const onCardNameSubmit: FormEventHandler = event => {
