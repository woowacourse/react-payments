...
const validators = {
  password(value: string): string {
    ...
    if (value.length !== CARD_CONSTRAINTS.PASSWORD.LENGTH) {
  ...
  cvc(value: string): string {
    ...
    if (value.length !== CARD_CONSTRAINTS.CVC.LENGTH) {
  ...
  cardExpiration(value: string, index: number): string {
    ...
    if (value.length !== CARD_CONSTRAINTS.CARD_EXPIRATION.LENGTH) {
      return `숫자 ${CARD_CONSTRAINTS.CARD_EXPIRATION.LENGTH}개를 정확히 입력해주세요.`;
    }
    if (index === 0 && !(Number(value) >= CARD_CONSTRAINTS.CARD_EXPIRATION.MIN_MONTH && Number(value) <= CARD_CONSTRAINTS.CARD_EXPIRATION.MAX_MONTH)) {
  ...
  cardNumbers(value: string): string {
    ...
    if (value.length !== CARD_CONSTRAINTS.CARD_NUMBERS.LENGTH) {
