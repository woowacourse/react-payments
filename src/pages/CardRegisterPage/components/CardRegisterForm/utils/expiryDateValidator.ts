...
import {
  ...
  MAX_EXPIRY_MONTH,
  MIN_EXPIRY_MONTH,
} from "../ExpiryDateField/constants";
...
export const validateExpiryMonth = (expriyMonth: string) => {
  ...
  if (!isMonthLengthValid(expriyMonth)) {
    ...
    errorInformation.errorMessage = ERROR_MESSAGES.EXPIRY_MONTH_INVALID_LENGTH;
  }

  if (!isMonthRangeValid(expriyMonth)) {
    errorInformation.isValid = false;
    errorInformation.errorMessage = ERROR_MESSAGES.EXPIRY_MONTH_INVALID_RANGE;
  }
...
export const validateExpiryYear = (expriyYear: string) => {
  ...
  if (!isYearLengthValid(expriyYear)) {
    errorInformation.isValid = false;
    errorInformation.errorMessage = ERROR_MESSAGES.EXPIRY_YEAR_INVALID_LENGTH;
  }
