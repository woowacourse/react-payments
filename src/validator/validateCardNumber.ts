...
const validateCardNumber = (cardNumber: string): ErrorDetail => {
  const isValidLength =
    cardNumber.length === CONDITION.TEXT_LENGTH_MIN ||
    cardNumber.length === CONDITION.CARD_NUMBER_LENGTH_MAX;
  const isValidCardNumber = /^\d{4}$/.test(cardNumber);

  if (!isValidLength) {
    return {
      isError: true,
      errorMessage: `카드 번호는 ${CONDITION.CARD_NUMBER_LENGTH_MAX}자리로 입력해주세요`,
    };
  }
