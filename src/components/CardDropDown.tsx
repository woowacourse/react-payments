...
export default function CardDropDown({ cardBrand, handleInput, handleShowComponent } : Props) {
  ...
  return (
    <> 
    <FieldTitle title="카드사를 선택해 주세요" subtitle="현재 국내 카드사만 가능합니다."/>
        ...
        view && (
          <>
            {Object.entries(CARD_BRAND).map(([brandName]) => {
              return <li key={brandName} value={brandName} onClick={() => handleSelectBrand(brandName)}>{brandName}</li>
            })}
          </>
        )
      }
