...
- [ ] select로 8개 카드사를 선택 할 수 있게 한다.
- [ ] 각 카드사에 맞는 CardPreview색을 실시간으로 변경시킨다.
- [ ] 카드사 select를 클릭하고 선택을 하지 않고 onBlur가 되면 border와 함께 에러메시지를 띄운다.

### 비밀번호 입력 컴포넌트

- [ ] 비밀번호 앞 2자리를 입력받는다.
- [ ] input에 입력값이 \*로 보이게 한다.
- [ ] onBlur시 입력값이 2개 미만이면 border 색 변경 및 에러메시지
