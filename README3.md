...
## 사전 인지할 점 (step2 리뷰)
- [ ] 조립 순서 정리: 섹션 훅 내부의 원자 훅 조립 순서를 의도가 드러나도록 재배치 (포커스 -> 입력 -> 에러).
- [ ] 성공 페이지 라우팅 주의 - 서버 연동 시에도 응답받은 식별자(`id`)와 렌더링에 필요한 안전한 데이터만 성공 페이지로 넘기도록 구조 유지.

## MSW
- [ ] POST `/cards` (카드 등록) 핸들러 구현
...
## 통합 테스트
- [ ] MSW와 RTL로 사용자 관점의 통합 테스트 작성 : 카드 등록 -> 목록 조회 -> 카드 삭제.
- [ ] 400 에러 발생 시 UI에 에러 메시지가 올바르게 노출되는지 테스트.